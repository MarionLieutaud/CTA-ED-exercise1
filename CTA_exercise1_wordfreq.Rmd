...
```{r}
edbf_term_counts_race <- tidy_des_genre %>% 
  group_by(year, genre) %>%
  count(word, sort = TRUE)

race_terms <- "race|racial|racism|racist|ethnic|ethnicity|diversity|minority|migration|migrant|immigrant|colonial|colonialism|slavery|slave|diaspora|refugee|asylum"

edbf_term_counts$raceword <- as.integer(grepl(race_terms,
                                             x = edbf_term_counts$word))
...
edbf_counts_gender <- edbf_counts %>%
  mutate(type = "Gender")

edbf_counts_race2 <- edbf_counts_race %>%
  rename(sum_wom = sum_race) %>%
  mutate(type = "Race")

both <- bind_rows(edbf_counts_gender, edbf_counts_race2)

ggplot(both, aes(year, sum_wom / year_total, color = type)) +
  geom_line() +
  ylab("% of keywords in descriptions") +
  scale_y_continuous(labels = scales::percent_format(),
                     expand = c(0, 0), limits = c(0, NA)) +
  theme_tufte(base_family = "Helvetica")
